...
#from exportaPdf import generatePDF
...
class SecondWindow(Toplevel):
    def __init__(self, master=None):
        ...
        lb_ficha = Entry(frame3)
        lb_ficha.place(relx=0.015, rely=0.51, relheight=0.05, relwidth=0.19)
        btn3 = Button(frame3, text='Ficha', command=lambda: generatePDF(lb_ficha.get()))
        btn3.place(relx=0.21, rely=0.505, relheight=0.07, relwidth=0.07)
...
class MainWindow(Frame):
    def __init__(self):
        ...
        self.master.geometry("%dx%d" % (largura_screen, altura_screen))
